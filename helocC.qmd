...
Since the ground truth conditional quantile distribution is given by
\begin{align}
  C(q_2 \mid q_1) = \text{Unif}[q_1, 1],
\end{align}
the $H_0$ hypothesis of a Frank copula quantile coupling is rejected. 
...
Finally, we fit a model that performs copula-free learning from Alg. 3, by regressing
\begin{align}
  V^*_i \sim \Pa^*_i + V_i + \Pa_i.
\end{align}
...
As another baseline model, we also consider the model of using only the post-recourse parents and recourse data, in which we regress
\begin{align}
  V^*_i \sim \Pa^*_i.
\end{align}
...
Finally, we compute the Kolmogorov-Smirnov (KS) statistics for the distance of the predicted MC empirical distributions to the underlying true distribution. In particular, we compute
\begin{align}
...
\end{align}
where $D_n(A, B)$ is the KS statistic computed as $\sup_{x \in \mathbb{R}} |F_A(x) - F_B(x)|$ where $F_A, F_B$ are empirical cumulative distribution functions of samples $A, B$.
